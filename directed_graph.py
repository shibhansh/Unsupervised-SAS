...
class Graph(object):
    ...
    def rank_pairs(self,ranks=[],weights=[],pairs_to_rank=10,to_remove_parent_transfer=False):
        if self.all_pairs_shortest_paths == {}:
            self.all_pairs_shortest_paths = dict(nx.all_pairs_shortest_path(self.undirected_nx_graph))
        ...
        for index_n1, n1 in enumerate(ranks):
            for index_n2, n2 in enumerate(ranks):
                if n1!=n2 and n2 in self.all_pairs_shortest_paths[n1].keys(): 
                    ...
                    pair_weights[n1+' '+n2] = total_weight
        ...
        # need to return only specific num of pairs; removing repetetive pairs
        rank_order_pairs = []
        ...
        for pair in ordered_pairs:
            ...
            if len(current_path):# and len(current_path) < 9: 
                ...
                if to_add_current_path:
                    paths_to_return.append([current_path,self.get_sub_graph(nodes=current_path)[1],sent])
        return paths_to_return

    def highest_rank_subgraph(self,ranks=[],weights=[]):
            selected_nodes = []
            for index, node in enumerate(ranks):
                if weights[index] > 2:
                    selected_nodes.append(node)

            selected_nodes = self.connect_unconnected_components(nodes=selected_nodes)
            selected_nodes, new_graph = self.get_sub_graph(nodes=selected_nodes)
            return new_graph

    def construct_summary_greedily(self,ranks=[],weights=[]):
        top_nodes = []
        for index, node in reversed(list(enumerate(ranks))):
            if weights[index] > 4:
                top_nodes.append(node)

        top_nodes = list(reversed(list(ranks[-7:])))

        selected_nodes = []
        while not (set(top_nodes) < set(selected_nodes) or set(top_nodes) == set(selected_nodes)):
            ...
            selected_nodes, success = self.connect_unconnected_components(nodes=selected_nodes)
            if not success:
                top_nodes.remove(new_node)
                selected_nodes.remove(new_node)
            print selected_nodes, top_nodes

        selected_nodes, new_graph = self.get_sub_graph(nodes=selected_nodes)
        return new_graph

    def construct_greedily_first(self,ranks=[],weights=[],use_true_sent_rank=False,num_nodes=0,range_num_nodes=0,
        ...
        # nodes added in order of their weight
        if len(top_nodes) <= 3:  top_nodes = list(reversed(list(ranks[-10:])))
        else: top_nodes = list(reversed(list(ranks[-10:])))
        selected_nodes = [top_nodes[0]]
        selected_sents = []

        while not (set(top_nodes) < set(selected_nodes) or set(top_nodes) == set(selected_nodes)):
            selected_nodes = list(selected_nodes)
            top_nodes = list(top_nodes)
            if len(selected_nodes) >= num_nodes - range_num_nodes:    
                break
            for node in top_nodes:
                if node not in selected_nodes:
                    new_node = node
            ...
            for node in top_nodes:
                ...
                if node not in selected_nodes:  continue

                first_var = node
                second_var = new_node

                if use_true_sent_rank:
                    common_sents = sorted(list(set(self.actual_incoming_sents[first_var]).intersection(self.actual_incoming_sents[second_var])))
                else:    
                    common_sents = sorted(list(set(self.var_to_sent[first_var]).intersection(self.var_to_sent[second_var])))
                ...
                if sent != -1:  success = True

                if sent not in selected_sents and success:
                    possible_sents.append(sent)
                    if sent == min(possible_sents):  paired_var = first_var

            if len(possible_sents) == 0:
                top_nodes.remove(new_node)
            elif possible_sents != []:
                selected_sent = min(possible_sents)
                for path in nx.all_simple_paths(
                    self.get_sub_graph(nodes=self.sent_to_var[selected_sent])[1].undirected_nx_graph,
                    source=paired_var, target=second_var):
                    current_path = [-1]
                    self.depth_dict[-1] = 100000 
                    if min([self.depth_dict[var] for var in path]) < min([self.depth_dict[var] for var in current_path]):
                        current_path = path
                        nodes_to_add = -1
                        if concept_relation_list != None: 
                            nodes_to_add = concept_relation_list.get_concepts_given_path(sent_index=selected_sent,path=path)
                        if nodes_to_add == -1:
                            nodes_to_add = self.sent_to_var[selected_sent]
                            continue

                if current_path == [-1]:  selected_nodes.extend(self.sent_to_var[selected_sent])
                else:   selected_nodes.extend(nodes_to_add)
                selected_sents.append(selected_sent)

            selected_sents = list(set(selected_sents))
            selected_nodes = list(set(selected_nodes))

        if len(selected_nodes) <= 1:
            selected_nodes.extend(self.sent_to_var[0]) 
            selected_sents.append(0)

        selected_nodes, new_graph = self.get_sub_graph(nodes=selected_nodes)
        connected_components = nx.connected_component_subgraphs(new_graph.undirected_nx_graph)
        ...
        selected_nodes = largest_component
    ...
    def rank_sent_in_degree(self,):
        ...
        new_list = []
        for key in self.sent_in_degree.keys():
            min_sent = 10000
            try:
                min_sent = min(self.var_to_sent[key])
            except: pass
            new_list.append((key,self.sent_in_degree[key],min_sent))

        new_list.sort(key=lambda t: (t[1],-t[2]))
        return new_list

    def rank_with_meta_nodes(self,var_freq_list,cn_freq_dict={},cn_sent_lists={},cn_var_dict={}):
        combined_list = []
        combined_list.extend(var_freq_list)
        for cn in cn_sent_lists.keys():
            if cn_sent_lists[cn] == []: continue
            combined_list.append((cn_var_dict[cn][0],cn_freq_dict[cn], min(cn_sent_lists[cn])) )
        combined_list.sort(key=lambda t: (t[1],-t[2]))
        return combined_list

    def add_idf_ranking(self,var_freq_list=[],default_idf=1,idf_vars={},num_vars_to_add=0):
        new_list = []
        len_var_freq_list = len(var_freq_list)
        for index, entry in enumerate(var_freq_list):
            idf_vars.setdefault(entry[0],default_idf)
            if len_var_freq_list - index < num_vars_to_add+1:
                new_list.append((entry[0],entry[1]*idf_vars[entry[0]],entry[2]))
            else:
                new_list.append((entry[0],entry[1],entry[2]))
        new_list.sort(key=lambda t: (t[1],-t[2]))
        return new_list

    def rank_true_sent_in_degree(self,phrases=[]):
        # get the set of proposed phrases in the story, and the focus in each of those phrases
        self.actual_incoming_sents = {}
        for var in self.var_to_sent.keys():
           self.actual_incoming_sents[var] = list(self.var_to_sent[var])

        for phrase in phrases:
            focus_index, phrase_indices, current_sent, sent_indices = phrase[0], phrase[1], phrase[2], phrase[3]

            phrase_vars = self.get_phrase_vars(indices=phrase_indices)

            try:  focus_var = self.text_index_to_var[str(focus_index)][0]
            except:   focus_var = ''
            if focus_var == '': continue

            for var in phrase_vars:
                var_found = False
                if var == focus_var: continue
                try:    self.actual_incoming_sents[var].remove(current_sent)
                except:  pass

                for index in sent_indices:
                    if index in phrase_indices: continue
                    try:
                        if var in self.text_index_to_var[index]:    var_found = True
                    except KeyError:    pass

            if var_found:
                self.actual_incoming_sents[var].append(current_sent)
                self.actual_incoming_sents[var] = list(set(self.actual_incoming_sents[var]))


        self.true_sent_in_degree = {}
        for var in self.actual_incoming_sents.keys():
            self.true_sent_in_degree[var] = len(self.actual_incoming_sents[var])
            if self.true_sent_in_degree[var] != self.sent_in_degree[var]:
                pass

        new_list = []
        for key in self.true_sent_in_degree.keys():
            min_sent = 10000
            try: min_sent = min(self.actual_incoming_sents[key])
            except: pass
            new_list.append((key,self.true_sent_in_degree[key],min_sent))

        new_list.sort(key=lambda t: (t[1],-t[2]))
        return new_list

    def get_phrase_vars(self,indices=[]):
        new_pharse_vars = []
        for index in indices:
            try:    new_var = self.text_index_to_var[str(index)][0]
            except: new_var = ''
            if new_var!= '':
                new_pharse_vars.append(new_var)

        return new_pharse_vars
    ...
    def merge_nodes_in_graph(self,first_var='',second_var='',debug=False):
        ...
        returned_value,first_var,second_var = \
                self.pre_merger_sanity_checks(first_var=first_var,second_var=second_var,debug=debug)
        ...
        arg_edge_set_first_var = []
        for child in set(self._graph[second_var]):
            current_edge_set = self.edge_lables[second_var+' '+child]
            arg_current_edge_set = [edge for edge in current_edge_set if edge.startswith(':ARG')]
            arg_edge_set_first_var.extend(arg_current_edge_set)

        connections_to_remove = []
        nodes_alignment_to_preserve = []
        for child in set(self._graph[second_var]):
            current_edge_set = self.edge_lables[second_var+' '+child]
            if ':name' in current_edge_set: nodes_alignment_to_preserve.append(child)
        ...
        for node in nodes_to_remove:
            if node in nodes_alignment_to_preserve:
                for index in self.text_index_to_var:
                    if node in self.text_index_to_var[index]:
                        self.text_index_to_var[index].remove(node)
                        self.text_index_to_var[index].append(first_var)
                        self.text_index_to_var[index] = list(set(self.text_index_to_var[index]))
            self.remove(node)
 
        self.merge_same_children(node=first_var,debug=debug)
    ...
    def pre_merger_sanity_checks(self,first_var='',second_var='',debug=False):
        ...
        for child_1 in self._graph[first_var]:
            edge_set_first_child = self.edge_lables[first_var+' '+child_1]
            for child_2 in self._graph[second_var]:
                edge_set_second_child = self.edge_lables[second_var+' '+child_2]
                common_edges = [edge for edge in edge_set_second_child if edge in edge_set_first_child]
                for edge in common_edges:
                    if edge.startswith(':ARG'):
                        if debug:   print 'common_text children', child_2, self.common_text[child_2], child_1, self.common_text[child_1]
                        if self.common_text[child_2] != self.common_text[child_1]:
    ...
    def merge_same_children(self,node='',debug=False):
        pairs_can_not_merge = []
        while True:
            found_nodes_to_merge = False
            for child_1 in self._graph[node]:
                for child_2 in self._graph[node]:
                    if child_1 == child_2:  continue

                    edge_set_first_child = self.edge_lables[node+' '+child_1]
                    edge_set_second_child = self.edge_lables[node+' '+child_2]

                    if len(list(set(edge_set_second_child).intersection(edge_set_first_child))) != 0:
                        if self.common_text[child_2] == self.common_text[child_1]:
                            if child_1 + ' ' + child_2 in pairs_can_not_merge:  continue
                            pairs_can_not_merge.append(child_1+' '+child_2)

                            self.merge_nodes_in_graph(first_var=child_1,second_var=child_2,debug=debug)
    ...
    def check_sent_var_mapping(self,):
    ...
    def get_sub_graph(self,nodes=[]):
        ...
        for node in selected_nodes:
            if self.depth_dict[node] > depth_root:
                root = node 
                depth_root = self.depth_dict[node]
